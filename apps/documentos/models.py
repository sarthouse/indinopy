...
class DocumentoAdjunto(TimeStampedModel):
    ...
    def _validar_tamano(self):
        if self.archivo and self.archivo.size > MAX_FILE_SIZE:
            raise ValidationError({
                'archivo': _(f'El archivo excede el tamaño máximo de {MAX_FILE_SIZE // (1024*1024)}MB')
            })
    ...
    def _validar_tipo(self):
        ...
        if ext not in ALLOWED_EXTENSIONS:
            raise ValidationError({
                'archivo': _(f'Tipo de archivo no permitido: {ext}. Permitidos: PDF, imágenes, Excel, Word')
            })

    def _validar_nombre_duplicado(self):
        if not self.pk or not self.res_model or not self.res_id:
            return
        ...
        if qs.exists():
            raise ValidationError({
                'nombre': _(f'Ya existe un archivo con el nombre "{self.nombre}" adjunto a este registro')
            })
